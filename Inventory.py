...
def Database():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS `product` (product_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, equipment_name TEXT, product_qty TEXT, borrow TEXT, product_price TEXT)")
...
def AddNewForm():
    ...
    lbl_qty = Label(MidAddNew, text="Borrower:", font=('arial', 25), bd=10)
    lbl_qty.grid(row=2, sticky=W)
    lbl_price = Label(MidAddNew, text="Equipment Quantity:", font=('arial', 25), bd=10)
    ...
    equipmentqty = Entry(MidAddNew, textvariable=STATUS, font=('arial', 25), width=15)
    equipmentqty.grid(row=1, column=1)
    BRR = Entry(MidAddNew, textvariable=BORROWER, font=('arial', 25), width=15)
    BRR.grid(row=2, column=1)
    status = Entry(MidAddNew, textvariable=EQUIPMENT_QTY, font=('arial', 25), width=15)
...
def AddNew():
    Database()
    cursor.execute("INSERT INTO `product` (equipment_name, status, borrower, equipment_qty) VALUES(?, ?, ?, ?)",
                   (str(EQUIPMENT_NAME.get()), str(STATUS.get()), str(BORROWER.get()), int(EQUIPMENT_QTY.get())))
    conn.commit()
    EQUIPMENT_NAME.set("")
    STATUS.set("")
    BORROWER.set("")
...
def ViewForm():
    ...
    tree = ttk.Treeview(MidViewForm, columns=("Equipment Name", "Status", "Borrower", "Equipment Qty"),
    ...
    tree.heading('Borrower', text="Borrower", anchor=W)
